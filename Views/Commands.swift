...
struct NavigationCommands: View {
    ...
    var body: some View {
        Button("button-go-back".localized) { browser?.webView.goBack() }
            .keyboardShortcut("[")
            .disabled(canGoBack != true)
        Button("button-go-forward".localized) { browser?.webView.goForward() }
            .keyboardShortcut("]")
...
struct PageZoomCommands: View {
    ...
    var body: some View {
        Button("title-actual-size".localized) { webViewPageZoom = 1 }
            .keyboardShortcut("0")
            .disabled(webViewPageZoom == 1 || browser?.url == nil)
        Button("title-zoom-in".localized) { webViewPageZoom += 0.1 }
            .keyboardShortcut("+")
            .disabled(webViewPageZoom >= 2 || browser?.url == nil)
        Button("title-zoom-out".localized) { webViewPageZoom -= 0.1 }
            .keyboardShortcut("-")
