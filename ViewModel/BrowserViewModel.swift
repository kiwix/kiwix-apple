...
final class BrowserViewModel: NSObject, ObservableObject,
    ...
    nonisolated static func destroyTabById(id: NSManagedObjectID) {
        Task { @MainActor in
            if let browserViewModel = cache?.findBy(key: id) {
                ...
                os_log(
                    "BrowserViewModel.Cache.destroyTabById, remaining tabs: %i",
                    log: Log.Browser,
                    type: .debug,
                    count
                )
    ...
    nonisolated static func keepOnlyTabsByIds(_ ids: Set<NSManagedObjectID>) {
        Task { @MainActor in
            if let cache {
                ...
                os_log(
                    "BrowserViewModel.Cache.keepOnlyTabsByIds: %s, remaining tabs: %i",
                    log: Log.Browser,
                    type: .debug,
                    tabIds, cache.count
                )
    ...
    @MainActor private init(tabID: NSManagedObjectID) {
        self.tabID = tabID
        self.tabIDDebug = "\(tabID)"
        os_log(
            "BrowserViewModel.init tabID: %s",
            log: Log.Browser,
            type: .debug,
            tabIDDebug
        )
    ...
    func destroy() async {
        ...
        #endif
        os_log(
            "BrowserViewModel.destroy tabID: %s",
            log: Log.Browser,
            type: .debug,
            tabIDDebug
        )
    ...
    func clear() async {
        ...
        isLoading = nil
        os_log(
            "BrowserViewModel.clear tabID: %s",
            log: Log.Browser,
            type: .debug,
            tabIDDebug
        )
    ...
    deinit {
        os_log(
            "BrowserViewModel.deinit tabID: %s",
            log: Log.Browser,
            type: .debug,
            tabIDDebug
        )
