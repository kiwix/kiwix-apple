...
class LibraryViewController2: UITableViewController, NSFetchedResultsControllerDelegate, DownloaderDelegate, BookCellDelegate {
    ...
    func performFetch() {
        // This if should not be needed, I believe it's a bug in iOS 9.0
...
//        }
    ...
    func didTapOnAccessoryViewForCell(atIndexPath indexPath: NSIndexPath?) {
        if segmentedControl.selectedSegmentIndex == 1 {
            let cell = tableView.cellForRowAtIndexPath(indexPath!) as! BookDownloadingCell
    ...
    lazy var downloadFetchedResultController: NSFetchedResultsController = {
        ...
        fetchRequest.predicate = NSPredicate(format: "totalBytesWritten != nil AND isLocal == NO", argumentArray: nil)
    ...
    func configureCell(cell: UITableViewCell, atIndexPath indexPath: NSIndexPath) {
        ...
        if cell.isKindOfClass(BookOrdinaryCell) {
            ...
            if let totalBytesWritten = Downloader.sharedInstance.totalBytesWrittenDic[book.idString!], fileSize = book.fileSize?.longLongValue {
                cell.progressView.setProgress(Float(totalBytesWritten/(fileSize * 1024)), animated: false)
                ...
                print("2.\(cell.progressView.progress)")
            } else {
    ...
    func downloadStateOfBook(book: Book) -> BookDownloadState {
        ...
        if book.isLocal == true {
            return .Finished
        } else if Downloader.sharedInstance.urlSessionDic[book.idString!] != nil {
            return BookDownloadState.CanPause
        } else if book.hasResumeData == true {
            return BookDownloadState.CanResume
        } else {
